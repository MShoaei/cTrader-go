...
func (client *Client) SendRequest(reqType openapi.ProtoOAPayloadType, resType []openapi.ProtoOAPayloadType, errType []openapi.ProtoOAPayloadType, req proto.Message, clientMsgUuid *uuid.UUID) (interface{}, error) {
	...
	_, err = client.conn.SendMessage(uint32(reqType), req, clientMsgUuid)
	if err != nil {
		return nil, err
	}
...
func (client *Client) handleMessage(b []byte) error {
	...
	if protoMessage.ClientMsgId == nil {
		log.Debug().
			Str("ProtoOA payload type", openapi.ProtoOAPayloadType_name[int32(*protoMessage.PayloadType)]).
			Str("Proto payload type", openapi.ProtoPayloadType_name[int32(*protoMessage.PayloadType)]).
			Interface("client msg id", protoMessage.ClientMsgId).
			Send()
	} else {
		log.Debug().
			Str("ProtoOA payload type", openapi.ProtoOAPayloadType_name[int32(*protoMessage.PayloadType)]).
			Str("Proto payload type", openapi.ProtoPayloadType_name[int32(*protoMessage.PayloadType)]).
			Str("client msg id", *protoMessage.ClientMsgId).
			Send()
	}
	...
	if protoMessage.ClientMsgId != nil && !strings.HasPrefix(*protoMessage.ClientMsgId, "ct") {
